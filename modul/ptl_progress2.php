                        ...
                        <tbody>
                            <?php 
                                ...
                                */
                                ...
                                $sql    = "SELECT * FROM m_user WHERE u_level = 2";
                                $query  = mysqli_query($konek, $sql);
                                $no     = 1;
                                while($data = mysqli_fetch_array($query)){ 
                                    $uid    = $data['u_id']; 
                                    ...
                                    $pap    = " SELECT 
                                                ...
                                                LEFT JOIN m_project ON m_crm2.c_id=m_project.p_c_id
                                                LEFT JOIN m_user ON m_crm2.c_pic=m_user.u_name_crm
                                                WHERE p_status IN (51,61)
                                                AND u_id = $uid";

                                    $nq  = mysqli_query($konek, $pan);
                                    $fnq = mysqli_fetch_array($nq);

                                    $cq  = mysqli_query($konek, $pac);
                                    $fcq = mysqli_fetch_array($cq);

                                    $pq  = mysqli_query($konek, $pap);
                                    $fpq = mysqli_fetch_array($pq);
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo $fnq['total']; ?></td>
                                        <td><?php echo $fcq['total']; ?></td>
                                        <td><?php echo $fpq['total']; ?></td>
                                        <td><?php echo sprintf("%0.1f", ($fpq['total'] / $fnq['total']) * 100) ?></td>
                                        <td>
                                            <a href="?modul=ptl_progress&id=<?php echo $data['u_id']?>">
