<?php
		...
		elseif($stat == 7){
			include('helper/count_baa_mks.php');
		}
		else{
						...
						<table class=" table table-hover general-table" id="dynamic-table">
							...
							<?php	
								...
								if($stat == 1){
									...
									WHERE p_status = 0
									AND c_sbu_owner = 'SBU RO MAKASAR'
									-- ORDER BY c_id ASC
								...
								elseif($stat == 5){
									$sql=  "SELECT *,
												...
												datediff(p_closed_date, c_created_date) as creates2  FROM m_crm2 
											LEFT JOIN m_project ON m_crm2.c_id=m_project.p_c_id
											LEFT JOIN m_status ON m_project.p_status=m_status.s_code
											LEFT JOIN m_user ON m_crm2.c_pic=m_user.u_name_crm
								...
								while($data = mysqli_fetch_array($query)){ ?>
									<tr>
										...
										<td><a href="?modul=detail&id=<?php echo $data['p_id'] ?>"><?php echo $data['p_bai_date'] ?></a></td>
